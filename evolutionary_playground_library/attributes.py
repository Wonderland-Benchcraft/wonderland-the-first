...
class AttributeGenome:
    ...
    def __init__(self,
        ...
        """
        self.max_hp = max_hp if max_hp is not None else random.uniform(80, 120)
        ...
        self.reproduction_chance = reproduction_chance if reproduction_chance is not None else random.uniform(0.0001, 75)
    def mutate(self, mutation_rate=0.2, mutation_strength=0.1):
        ...
        self.aging_coeff = max(0.001, min(self.aging_coeff, 0.05))
        ...
        self.reproduce_cooldown = max(10, min(self.reproduce_cooldown, 100))
        self.reproduce_hp_damage =  max(self.max_hp*0.1, min(self.reproduce_hp_damage, self.max_hp))
        self.reproduction_chance = max(0.0001, min(self.reproduction_chance, 75))
